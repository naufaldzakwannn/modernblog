...
export default function Aside() {
  return (
    <aside className="py-20 px-6 max-w-7xl mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 border border-ink/10">
          ...
          <div className="space-y-2">
            {topics.map((t) => (
              <Link
                key={t.name}
                href="#"
                className="flex items-center justify-between py-2 border-b border-ink/5 group"
              >
                <span className="font-body text-sm text-ink/70 group-hover:text-ink transition-colors">
                  {t.name}
                </span>
        ...
        <div className="p-8 border-b lg:border-b-0 lg:border-r border-ink/10 bg-ink flex flex-col justify-center">
          ...
          <blockquote className="font-display text-2xl italic text-paper/90 leading-tight">
            Ideas are the currency of the future.
          </blockquote>
          <cite className="font-mono text-[10px] tracking-widest uppercase text-paper/30 mt-4 not-italic">
            — The Editorial
          </cite>
        </div>
        ...
        <div id="subscribe" className="p-8">
          ...
          <p className="font-body text-sm text-ink/60 leading-relaxed mb-6">
            Essays, ideas, and perspectives delivered to your inbox every two weeks. No noise.
          </p>
          <div className="space-y-3">
            <input
              type="email"
              placeholder="your@email.com"
              className="w-full bg-cream border border-ink/15 px-4 py-3 font-body text-sm text-ink placeholder:text-ink/30 focus:outline-none focus:border-rust transition-colors"
            />
            <button className="w-full bg-rust text-paper font-mono text-xs tracking-widest uppercase py-3 hover:bg-ink transition-colors">
              Subscribe Free
            </button>
          </div>
          <p className="font-mono text-[9px] uppercase tracking-widest text-ink/25 mt-3">
